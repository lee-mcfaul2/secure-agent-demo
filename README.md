...
fresh clone has nothing under `chart/charts/`. Dependency resolution needs
the five upstream repos registered first or it fails with
`no repository definition for https://helm.linkerd.io/stable, ...`. One
dependency (`agent-sql-mcp`) is also a local patched override under
`chart/charts-local/`, so `Chart.lock` is regenerated by
`helm dependency update` rather than committed.
...
script are committed in the chart; `Chart.lock` is regenerated at install
time (`helm dependency update`) because of the local `agent-sql-mcp`
override.
